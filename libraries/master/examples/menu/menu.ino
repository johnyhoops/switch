...
void setup() {
  ...
  for(int i = 1 ; i <= NUMBEROFSLAVES ; i++){
    slave[i].setSlaveAddress(i);
    slave[i].attachMenu(menus[i]);
  }
}
...
void loop() {
  ...
  // update logic
  if(temperature > setPoint + hysteresis) slave[1].set(Output1, On);
  if(temperature < setPoint - hysteresis) slave[1].set(Output1, Off);
