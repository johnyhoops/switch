#include <avr/wdt.h>

#include "switch.h"

// declare global variables
int16_t temperature1;
int16_t temperature2;
int16_t pressure;
int16_t count;
int16_t pumpStatus = 0;
int16_t setPoint = 200;
int16_t hysteresis = 10;
int16_t test = 8888;

// declare menus
MenuItem menuItems[] = {
  {"8.8.8.8", Unit, ReadOnly, &test},
  {"T 1", Deci, ReadOnly, &temperature1},
  {"T 2", Deci, ReadOnly, &temperature2},
  {"Pump", Bool, ReadWrite, &pumpStatus},
  {"pres", Centi, ReadOnly, &pressure},
  {"cnt", Unit, ReadOnly, &count},
  {"Set", Deci, ReadWrite, &setPoint},
  {"Hyst", Deci, ReadWrite, &hysteresis},
  {0}   // must end with a zero to indicate end of list
};

void setup() 
{
    wdt_enable(WDTO_2S);
    Switch.begin();
    Switch.addMenu(menuItems);
}

void loop() {
  // update measurements
  temperature1 = Switch.get(Philips1);
  temperature2 = Switch.get(Philips2);
  pressure = Switch.get(InputI2C);
  count = Switch.get(Counter2);
  
  // update logic
  if(pumpStatus) {
    Switch.set(Output1, pwm_On);
    Switch.set(Output2, pwm_Off);
  } else {
    Switch.set(Output2, pwm_On);
    Switch.set(Output1, pwm_Off);
  }
 

  // update switch
  Switch.update();
  
  // kick the dog
  wdt_reset();
  
}
